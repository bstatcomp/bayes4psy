...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(46, 44, "end", "model_reaction_time");
    return reader;
}
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        try {
            ...
            for (int j = 1; j <= m; ++j) {
                ...
                lp_accum__.add(normal_log<propto__>(get_base1(mu,j,"mu",1), mu_m, sigma_m));
                if (get_base1(mu,j,"mu",1) < 0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-normal_ccdf_log(0, mu_m, sigma_m));
                current_statement_begin__ = 26;
                lp_accum__.add(normal_log<propto__>(get_base1(sigma,j,"sigma",1), mu_s, sigma_s));
                if (get_base1(sigma,j,"sigma",1) < 0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-normal_ccdf_log(0, mu_s, sigma_s));
                current_statement_begin__ = 27;
                lp_accum__.add(normal_log<propto__>(get_base1(lambda,j,"lambda",1), mu_l, sigma_l));
            }
