...
for (a in age_groups) {
  ...
  # fit
  fit <- b_reaction_time(rt=df_age$rt, s=df_age$subject_id, chains=1)
...
df_means <- df_summary %>% group_by(age_group) %>%
  summarize(mean_mu=mean(mu), low=quantile(mu, 0.025), high=quantile(mu,0.975))

ggplot(data=df_means, aes(x=age_group, y=mean_mu, group=1)) +
  geom_ribbon(aes(ymin=low, ymax=high), fill="#000000", alpha=0.2) +
  geom_line(color="#000000")

ggplot(data=df_summary, aes(x=age_group, y=mu, group=1)) +
